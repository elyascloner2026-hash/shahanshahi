...
CREATE TABLE IF NOT EXISTS battles (
    ...
    winner_id BIGINT NOT NULL REFERENCES users(user_id),
...
CREATE TABLE IF NOT EXISTS group_settings (
    ...
    hero_event_last_at INTEGER NOT NULL DEFAULT 0
);
...
class PGCursor:
    def __init__(self, rows=None, status=""):
        self.rows = list(rows or [])
        self.index = 0
        self.status = status
        try:
            self.rowcount = int(status.rsplit(" ", 1)[1])
...
class PGCompat:
    ...
    async def execute(self, sql, args=()):
        ...
        if stripped.startswith("SELECT"):
            ...
            return PGCursor(rows, f"SELECT {len(rows)}")

        if " RETURNING " in stripped:
            rows = await self.conn.fetch(sql, *args)
            command = stripped.split()[0]
            return PGCursor(rows, f"{command} {len(rows)}")

        status = await self.conn.execute(sql, *args)
        return PGCursor([], status)
...
class Database:
    ...
    def __init__(self, url: str = DATABASE_URL):
        self.url = url
        self._conn: PGCompat | None = None

    async def connect(self):
        raw = await asyncpg.connect(self.url)
        self._conn = PGCompat(raw)
        await self._conn.executescript(SCHEMA)
        await self._ensure_hero_group_column()
        await self._ensure_hero_event_chat_column()

        # Telegram group/supergroup chat IDs can exceed PostgreSQL INTEGER.
        # Keep them as BIGINT.
        await self._conn.execute(
            "ALTER TABLE battles ALTER COLUMN chat_id TYPE BIGINT"
        )
        await self._conn.execute(
            "ALTER TABLE group_settings ALTER COLUMN chat_id TYPE BIGINT"
        )
        await self._conn.execute(
            "ALTER TABLE group_battles ALTER COLUMN chat_id TYPE BIGINT"
        )
        # One-time Gem economy migration.
        ...
        if not marker:
            if ADMIN_IDS:
                placeholders = ",".join("$1" for _ in ADMIN_IDS)
                await self._conn.execute(
    ...
    async def _seed_defaults(self):
        ...
        if row["c"] == 0:
            await self._conn.executemany(
                "INSERT INTO territories (name, emoji) VALUES ($1, $2)", TERRITORIES
            )
    ...
    async def ensure_user(self, user_id: int, username: str | None):
        await self._conn.execute(
            "INSERT INTO users (user_id, username, joined_at) VALUES ($1, $2, $3) "
    ...
    async def set_banned(self, user_id: int, banned: bool):
        await self._conn.execute(
            "UPDATE users SET is_banned = $1 WHERE user_id = $2", (int(banned), user_id)
        )
        await self._conn.commit()
    ...
    async def create_character(self, user_id, name, gender, territory, city):
        ...
        if user_id in ADMIN_IDS:
            await self._conn.execute(
                "UPDATE characters SET gems = 1000000000 WHERE user_id = $1", (user_id,)
            )
            await self._conn.commit()
        # A new player starts with a small basic army.
        await self._conn.executemany(
            "INSERT INTO army_units (user_id, unit_code, quantity, max_quantity) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id, unit_code) DO NOTHING",
    ...
    async def update_character(self, user_id: int, **fields):
        if not fields:
            return
        cols = ", ".join(f"{k} = $1" for k in fields)
        values = list(fields.values()) + [user_id]
        await self._conn.execute(
            f"UPDATE characters SET {cols} WHERE user_id = $1", values
        )
        await self._conn.commit()
    ...
    async def list_cities(self, territory_name: str):
        cur = await self._conn.execute(
            "SELECT * FROM cities WHERE territory_name = $1 ORDER BY id",
            (territory_name,),
        )
        return await cur.fetchall()
    ...
    async def add_coins(self, user_id: int, amount: int, reason: str):
        await self._conn.execute(
            "UPDATE characters SET coins = GREATEST(0, coins + $1) WHERE user_id = $2",
            (amount, user_id),
        )
        await self._conn.execute(
            "INSERT INTO transactions (user_id, amount, reason, created_at) VALUES ($1, $2, $3, $4)",
    ...
    async def add_xp(self, user_id: int, amount: int):
        await self._conn.execute(
            "UPDATE characters SET xp = xp + $1 WHERE user_id = $2", (amount, user_id)
        )
        await self._conn.commit()

    async def add_gems(self, user_id: int, amount: int, reason: str = "unknown"):
        await self._conn.execute(
            "UPDATE characters SET gems = GREATEST(0, gems + $1) WHERE user_id = $2", (amount, user_id)
        )
        await self._conn.commit()

    async def spend_gems(self, user_id: int, amount: int) -> bool:
        cur = await self._conn.execute(
            "UPDATE characters SET gems = gems - $1 WHERE user_id = $2 AND gems >= $3",
    ...
    async def buy_gems_from_admin(self, buyer_id: int, admin_id: int, gems: int, coin_price: int) -> bool:
        ...
        if buyer_id == admin_id or gems <= 0 or coin_price <= 0:
            return False
        cur = await self._conn.execute(
            "UPDATE characters SET coins = coins - $1 WHERE user_id = $2 AND coins >= $3",
        ...
        if cur.rowcount != 1:
            ...
            return False
        await self._conn.execute(
            "UPDATE characters SET gems = gems + $1 WHERE user_id = $2", (gems, buyer_id)
        )
        await self._conn.execute(
            "UPDATE characters SET coins = coins + $1 WHERE user_id = $2", (coin_price, admin_id)
        )
        await self._conn.execute(
            "INSERT INTO transactions (user_id, amount, reason, created_at) VALUES ($1, $2, $3, $4)",
    ...
    async def transfer_gems(self, sender_id: int, receiver_id: int, amount: int) -> bool:
        ...
        if cur.rowcount != 1:
            ...
            return False
        cur = await self._conn.execute(
            "UPDATE characters SET gems = gems + $1 WHERE user_id = $2",
            (amount, receiver_id),
        )
        if cur.rowcount != 1:
            await self._conn.rollback()
    ...
    async def has_premium(self, user_id: int, code: str) -> bool:
        cur = await self._conn.execute(
            "SELECT 1 FROM premium_unlocks WHERE user_id = $1 AND code = $2", (user_id, code)
        )
    ...
    async def unlock_premium(self, user_id: int, code: str, price: int) -> bool:
        ...
        if cur.rowcount != 1:
            ...
            return False
        await self._conn.execute(
            "INSERT INTO premium_unlocks (user_id, code, unlocked_at) VALUES ($1, $2, $3)",
    ...
    async def get_premium(self, user_id: int):
        cur = await self._conn.execute(
            "SELECT code FROM premium_unlocks WHERE user_id = $1 ORDER BY code", (user_id,)
        )
    ...
    async def get_item(self, item_id: int):
        cur = await self._conn.execute("SELECT * FROM items WHERE id = $1", (item_id,))
        return await cur.fetchone()
    ...
    async def add_inventory_item(self, user_id: int, item_id: int):
        ...
        if row:
            await self._conn.execute(
                "UPDATE inventory SET quantity = quantity + 1 WHERE id = $1", (row["id"],)
            )
        else:
            await self._conn.execute(
    ...
    async def get_inventory(self, user_id: int):
        cur = await self._conn.execute(
            ...
            "JOIN items ON items.id = inventory.item_id WHERE inventory.user_id = $1",
            (user_id,),
        )
        return await cur.fetchall()
    ...
    async def get_army(self, user_id: int):
        cur = await self._conn.execute(
            "SELECT * FROM army_units WHERE user_id = $1 AND quantity > 0 ORDER BY unit_code", (user_id,)
        )
        return await cur.fetchall()

    async def get_all_army(self, user_id: int):
        cur = await self._conn.execute(
            "SELECT * FROM army_units WHERE user_id = $1 ORDER BY unit_code", (user_id,)
        )
        return await cur.fetchall()

    async def add_units(self, user_id: int, unit_code: str, amount: int):
        await self._conn.execute(
            "INSERT INTO army_units (user_id, unit_code, quantity, max_quantity) VALUES ($1, $2, $3, $4) "
            "ON CONFLICT(user_id, unit_code) DO UPDATE SET quantity = quantity + excluded.quantity, max_quantity = GREATEST(max_quantity, quantity + excluded.quantity)",
    ...
    async def set_units(self, user_id: int, unit_code: str, amount: int):
        await self._conn.execute(
            "INSERT INTO army_units (user_id, unit_code, quantity, max_quantity) VALUES ($1, $2, $3, $4) "
            "ON CONFLICT(user_id, unit_code) DO UPDATE SET quantity = GREATEST(0, excluded.quantity), max_quantity = MAX(max_quantity, excluded.quantity)",
    ...
    async def lose_army(self, user_id: int, ratio: float):
        ...
        for row in rows:
            ...
            await self._conn.execute(
                "UPDATE army_units SET quantity = GREATEST(0, quantity - $1) WHERE user_id = $2 AND unit_code = $3",
    ...
    async def heal_army(self, user_id: int):
        await self._conn.execute(
            "UPDATE army_units SET quantity = max_quantity WHERE user_id = $1", (user_id,)
        )
        await self._conn.commit()
        return await self.get_all_army(user_id)
    ...
    async def get_heal_slots(self, user_id: int):
        cur = await self._conn.execute(
            "SELECT heal_1_used_at, heal_2_used_at FROM characters WHERE user_id = $1", (user_id,)
        )
        return await cur.fetchone()

    async def use_heal_slot(self, user_id: int, slot: int, now: int):
        col = "heal_1_used_at" if slot == 1 else "heal_2_used_at"
        await self._conn.execute(f"UPDATE characters SET {col} = $1 WHERE user_id = $2", (now, user_id))
        await self._conn.commit()

    async def get_heroes(self, user_id: int):
        cur = await self._conn.execute(
            "SELECT * FROM heroes WHERE user_id = $1 ORDER BY id DESC", (user_id,)
        )
        return await cur.fetchall()

    async def count_heroes(self, user_id: int) -> int:
        cur = await self._conn.execute(
            "SELECT COUNT(*) as c FROM heroes WHERE user_id = $1", (user_id,)
        )
        row = await cur.fetchone()
        return row["c"]

    async def add_hero(self, user_id, name, rarity, power, defense, bonus):
        if await self.count_heroes(user_id) >= 3:
            return False
    ...
    async def record_battle(self, attacker_id, defender_id, winner_id, coins_reward, xp_reward, chat_id=None):
        ...
        battle_id = (await cur.fetchone())["id"]
        if chat_id is not None:
            await self._conn.execute(
                "INSERT INTO group_battles (battle_id, chat_id) VALUES ($1, $2)",
                (battle_id, chat_id),
            )
        await self._conn.commit()
        return battle_id
    ...
    async def set_last_fight(self, user_id: int, ts: int):
        await self._conn.execute(
            "UPDATE characters SET last_fight_at = $1 WHERE user_id = $2", (ts, user_id)
        )
        await self._conn.commit()
    ...
    async def _ensure_hero_group_column(self):
        try:
            await self._conn.execute(
                "ALTER TABLE group_settings ADD COLUMN hero_event_last_at INTEGER NOT NULL DEFAULT 0"
            )
            await self._conn.commit()
        except Exception:
            pass

    async def _ensure_hero_event_chat_column(self):
        try:
            await self._conn.execute(
                "ALTER TABLE hero_events ADD COLUMN chat_id BIGINT"
            )
            await self._conn.commit()
        except Exception:
            pass
    ...
    async def list_groups(self):
        cur = await self._conn.execute(
            "SELECT chat_id, title, hero_event_last_at FROM group_settings ORDER BY chat_id"
        )
        return await cur.fetchall()

    async def ensure_group(self, chat_id: int, title: str | None):
        await self._conn.execute(
            "INSERT INTO group_settings (chat_id, title, hero_event_last_at) "
            "VALUES ($1, $2, 0) "
    ...
    async def set_group_hero_event_time(self, chat_id: int, ts: int):
        await self._conn.execute(
            "UPDATE group_settings SET hero_event_last_at = $1 WHERE chat_id = $2",
            (ts, chat_id),
        )
        await self._conn.commit()
    ...
    async def get_active_hero_event(self, chat_id=None):
        if chat_id is None:
            cur = await self._conn.execute(
                "SELECT * FROM hero_events WHERE status = 'active' "
                "ORDER BY id DESC LIMIT 1"
            )
        else:
            cur = await self._conn.execute(
                "SELECT * FROM hero_events WHERE status = 'active' "
                "AND chat_id = $1 ORDER BY id DESC LIMIT 1",
                (chat_id,),
            )
        return await cur.fetchone()

    async def create_hero_event(self, name, hp_max, power, rarity, chat_id=None):
        cur = await self._conn.execute(
            "INSERT INTO hero_events "
            "(chat_id, name, hp_max, hp_current, power, rarity, created_at) "
            "VALUES ($1, $2, $3, $3, $4, $5, $6) RETURNING id",
            (chat_id, name, hp_max, power, rarity, int(time.time())),
        )
        await self._conn.commit()
    ...
    async def apply_hero_damage(self, event_id: int, user_id: int, damage: int):
        await self._conn.execute(
            "INSERT INTO hero_damage (event_id, user_id, damage, created_at) VALUES ($1, $2, $3, $4)",
            (event_id, user_id, damage, int(time.time())),
        )
        await self._conn.execute(
            "UPDATE hero_events SET hp_current = GREATEST(0, hp_current - $1) WHERE id = $2",
            (damage, event_id),
        )
        await self._conn.commit()
    ...
    async def finish_hero_event(self, event_id: int):
        await self._conn.execute(
            "UPDATE hero_events SET status = 'finished', finished_at = $1 WHERE id = $2",
    ...
    async def hero_event_damage_ranking(self, event_id: int):
        cur = await self._conn.execute(
            "SELECT user_id, SUM(damage) as total FROM hero_damage WHERE event_id = $1 "
    ...
    async def get_quest(self, code: str):
        cur = await self._conn.execute("SELECT * FROM quests WHERE code = $1", (code,))
        return await cur.fetchone()
    ...
    async def has_completed_quest_today(self, user_id: int, quest_id: int) -> bool:
        ...
        cur = await self._conn.execute(
            "SELECT COUNT(*) as c FROM quest_progress WHERE user_id = $1 AND quest_id = $2 AND completed_at > $3",
    ...
    async def has_completed_quest_ever(self, user_id: int, quest_id: int) -> bool:
        cur = await self._conn.execute(
            "SELECT COUNT(*) as c FROM quest_progress WHERE user_id = $1 AND quest_id = $2",
            (user_id, quest_id),
        )
        row = await cur.fetchone()
        return row["c"] > 0
    ...
    async def add_news(self, title: str, content: str, admin_id: int):
        await self._conn.execute(
            "INSERT INTO news (title, content, admin_id, created_at) VALUES ($1, $2, $3, $4)",
    ...
    async def latest_news(self, limit: int = 5):
        cur = await self._conn.execute(
            "SELECT * FROM news ORDER BY id DESC LIMIT $1", (limit,)
        )
        return await cur.fetchall()
